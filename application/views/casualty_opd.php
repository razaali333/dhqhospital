					...
					<div class="span12">
						<div class="widget-box">
							<div class="widget-header">
								<h4>Casualty OPD Informations</h4>
							</div>
							<div class="widget-body">
								<div class="widget-main no-padding">
									<form name="form" action="" method="post">
										<fieldset>
                                                      <?php $invno=$casualty_inv_no->row_array(); ?> 
                                                        <div class="span2">
														<label><strong>Recept #</strong></label>
														<input type="text" class="span12" id="receptNumber" name="receptNumber" value="Casu-<?php echo $invno['receptNumber']+1 ?>" readonly/>
                                                        </div>
                                                    <div class="span4">
...
document.onkeydown = function(event) {
	...
	$('#save').on('click',function(e){
		...
		$.ajax({
			url:'<?php echo base_url('login/insert_casualty_opd') ?>',
			method:'POST',
			data:{receptNumber:receptNumber,name:name,address:address,age:age,shift:shift,gander:gander},
			success:function()
			{
				window.open('<?php echo base_url('login/casualty_opd_print') ?>');
			location.reload();
			}
		})
	})
